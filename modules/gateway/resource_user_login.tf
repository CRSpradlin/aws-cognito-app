...
resource "aws_lambda_permission" "lambda_signInUser_method_post_user_login_permission" {
  ...
  source_arn = "arn:aws:execute-api:us-east-1:${var.str_aws_account_id}:${aws_api_gateway_rest_api.api.id}/*/${aws_api_gateway_method.method_post_user_login.http_method}${aws_api_gateway_resource.resource_user_login.path}"
}